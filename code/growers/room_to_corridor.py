...
class RoomToCorridorGeometryPlanner(GeometryPlanner[RoomToCorridorCandidate, RoomToCorridorPlan]):
    ...
    def plan(
        ...
        existing_links = set(generator.room_corridor_links)
    ...
    def _build_plan_for_option(
        ...
        target_corridor = generator.corridors[target_corridor_idx]
        ...
        if placement is None:
            ...
            print(requirements)
            generator.draw_to_grid()
            for x, y in junction_tiles:
                generator.grid[y][x] = "*"
            generator.mark_room_interior_on_grid(candidate.room_idx)
...
class RoomToCorridorApplier(GrowerApplier[RoomToCorridorCandidate, RoomToCorridorPlan]):
    ...
    def apply(
        ...
        component_id = generator._merge_components(
            generator._normalize_room_component(candidate.room_idx),
            generator._normalize_corridor_component(plan.target_corridor_idx),
        )
        generator._set_room_component(candidate.room_idx, component_id)
        generator._set_corridor_component(plan.target_corridor_idx, component_id)
        ...
        new_corridor_idx = generator._register_corridor(new_corridor, component_id)
        generator.placed_rooms[candidate.room_idx].connected_port_indices.add(candidate.port_idx)
        generator.placed_rooms[junction_room_index].connected_port_indices.add(branch_port_idx)
        generator.room_corridor_links.add((candidate.room_idx, new_corridor_idx))
        generator.room_corridor_links.add((candidate.room_idx, plan.target_corridor_idx))
        ...
        for idx in linked_indices:
            generator.room_corridor_links.add((candidate.room_idx, idx))
